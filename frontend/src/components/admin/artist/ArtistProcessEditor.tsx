...
export default function ArtistProcessEditor() {
  const [items, setItems] = useState<ArtistProcess[]>([]);
  ...
  async function loadItems() {
    try {
      ...
      const data = await getArtistProcess();
  ...
  async function handleSave(item: ArtistProcess) {
    try {
      setSaving(item.id);
      setMessage("");

      await saveArtistProcess({
  ...
  async function handleDelete(item: ArtistProcess) {
    ...
    try {
      ...
      await deleteArtistProcess(item.id);
  ...
  if (loadError) {
    return (
      <ErrorState
        ...
        message={
          loadError ||
          "We couldn't load the artist process. Please check your internet connection and try again."
        }
